...
def company_context(cid):
    return {"allowed_company_ids": [cid], "default_company_id": cid, "force_company": cid}
...
def main():
    ...
    def create_out_invoice(suffix=""):
        ...
        return retry(f"create out_invoice{suffix}", lambda: execute(models, uid, "account.move", "create", vals, context=company_context(company_a_id)))
    ...
    retry("action_post", lambda: execute(models, uid, "account.move", "action_post", [post_id], context=company_context(company_a_id)))
