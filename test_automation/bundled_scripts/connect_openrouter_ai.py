...
# -*- coding: utf-8 -*-
"""
Standalone RPC / HTTP test for connect_openrouter_ai (Odoo 19).

No odoo-bin / shell required. Run with plain Python 3:

    python3 models/test_connect_openrouter_ai_rpc.py
    python3 models/test_connect_openrouter_ai_rpc.py --protocol xmlrpc
    python3 models/test_connect_openrouter_ai_rpc.py --url http://localhost:8069 --db odoo --user admin --password admin

Live OpenRouter AI automation (requires valid API key in Odoo or via --openrouter-key):

    python3 models/test_connect_openrouter_ai_rpc.py --live
    python3 models/test_connect_openrouter_ai_rpc.py --live --openrouter-key "$OPENROUTER_API_KEY"

Custom prompt + context (your own QA scenario):

    python3 models/test_connect_openrouter_ai_rpc.py --live --custom-only \\
...
MODULE_NAME = "connect_openrouter_ai"
DEPENDENCY_MODULES = ("ai", "ai_app")

# ---------------------------------------------------------------------------
# Technical names from connect_openrouter_ai
# ---------------------------------------------------------------------------
PARAM_OPENROUTER_KEY = "connect_openrouter_ai.api_key"

MODEL_CONFIG_SETTINGS = "res.config.settings"
FIELD_OPENROUTER_KEY = "openrouter_api_key"
FIELD_OPENROUTER_KEY_ENABLED = "openrouter_key_enabled"
...
DEFAULT_LIVE_LLM = "openai/gpt-oss-120b:free"
PARAM_DEFAULT_LLM = "connect_openrouter_ai.default_llm_model"
FIELD_OPENROUTER_DEFAULT_LLM = "openrouter_default_llm"
...
class ConnectOpenRouterAIRPCTest:
    """End-to-end connect_openrouter_ai QA via RPC and HTTP."""
    ...
    def __init__(
        ...
        *,
        ...
        self.failed = 0
        self.skipped = 0
        self._saved_openrouter_key: str | False = False
    ...
    def _backup_and_set_openrouter_key(self, key: str | None) -> None:
        self._saved_openrouter_key = self._get_config_param(PARAM_OPENROUTER_KEY)
        if key:
            self._set_config_param(PARAM_OPENROUTER_KEY, key)

    def _restore_openrouter_key(self) -> None:
        if self._saved_openrouter_key is False:
            return
        self._set_config_param(PARAM_OPENROUTER_KEY, self._saved_openrouter_key or "")

    def _resolve_agent_id(self, name: str = "RPC Custom OpenRouter Agent") -> int:
    ...
    def _ensure_live_ready(self) -> None:
        ...
        current_key = self._get_config_param(PARAM_OPENROUTER_KEY)
        if self.openrouter_key:
            self._backup_and_set_openrouter_key(self.openrouter_key)
        elif current_key:
            self.openrouter_key = str(current_key)
        else:
            self._skip(
                "Live / custom AI tests",
                "no OpenRouter API key in Odoo and none passed via --openrouter-key",
            )
            self.live = False
    ...
    def _run_structural_tests(self) -> None:
        ...
        for model_name in OPENROUTER_LLM_MODELS:
            self._ok(
        ...
        self._ok(
            f"{MODEL_AI_EMBEDDING}.{FIELD_EMBEDDING_MODEL} includes {OPENROUTER_EMBEDDING_MODEL!r}",
            OPENROUTER_EMBEDDING_MODEL in embed_selection,
        )

        settings_fields = self.client.fields_get(
            MODEL_CONFIG_SETTINGS,
            [FIELD_OPENROUTER_KEY, FIELD_OPENROUTER_KEY_ENABLED, FIELD_OPENROUTER_DEFAULT_LLM],
        )
        self._ok(
            f"{MODEL_CONFIG_SETTINGS}.{FIELD_OPENROUTER_KEY} exists",
            FIELD_OPENROUTER_KEY in settings_fields,
        )
        self._ok(
            f"{MODEL_CONFIG_SETTINGS}.{FIELD_OPENROUTER_KEY_ENABLED} exists",
            FIELD_OPENROUTER_KEY_ENABLED in settings_fields,
        )
        self._ok(
            f"{MODEL_CONFIG_SETTINGS}.{FIELD_OPENROUTER_DEFAULT_LLM} exists",
            FIELD_OPENROUTER_DEFAULT_LLM in self.client.fields_get(
                MODEL_CONFIG_SETTINGS,
                [FIELD_OPENROUTER_DEFAULT_LLM],
            ),
        )
        ...
        agent_id = self._create_mistral_agent("RPC Structural OpenRouter Agent")
        ...
        self._ok(
            f"{MODEL_AI_AGENT} accepts OpenRouter LLM model",
    ...
    def _run_config_tests(self) -> None:
        ...
        self._backup_and_set_openrouter_key(test_key)
        try:
            stored = self._get_config_param(PARAM_OPENROUTER_KEY)
            self._ok(
                f"{PARAM_OPENROUTER_KEY} persists via ir.config_parameter",
                stored == test_key,
                f"got {stored!r}",
            )
        finally:
            self._restore_openrouter_key()

        current_key = self._get_config_param(PARAM_OPENROUTER_KEY)
        if self.live and self.openrouter_key:
            self._backup_and_set_openrouter_key(self.openrouter_key)
    ...
    def _run_missing_key_test(self) -> None:
        ...
        self._backup_and_set_openrouter_key("")
        try:
            try:
                ...
                self._ok(
                    "get_direct_response fails without OpenRouter API key",
                    False,
                ...
                self._ok(
                    "get_direct_response fails without OpenRouter API key",
                    "No API key set for OpenRouter" in str(exc),
                    str(exc)[:120],
                )
        finally:
            self._restore_openrouter_key()
            if self.live and self.openrouter_key:
                self._set_config_param(PARAM_OPENROUTER_KEY, self.openrouter_key)
    ...
    def _run_live_context_tests(self) -> None:
        ...
        agent_id = self._create_mistral_agent("RPC Context OpenRouter Agent")
        context_message = (
    ...
    def _run_live_rag_tests(self) -> None:
        ...
        agent_id = self._create_mistral_agent("RPC RAG OpenRouter Agent")
        rag_body = (
        ...
        try:
            ...
            indexed = self._wait_for_source_indexed(source_id)
            self._ok(
                "TC-RAG-02 source indexed with OpenRouter embeddings",
                indexed,
    ...
    def _run_live_record_context_tests(self) -> None:
        ...
        partner_name = "RPC OpenRouter Context Partner"
        partner_city = "Lyon"
    ...
    def _run_live_chat_style_tests(self) -> None:
        ...
        print("\n--- Live chat style (no Conversation Summary wrapper) ---")

        agent_id = self._create_mistral_agent("RPC Chat Style Agent")
        prompt = "hey"
        ...
        try:
            ...
            lowered = text.lower()
            self._ok(
                "TC-CHAT-01 greeting returns a non-empty reply",
                bool(text.strip()),
                text[:160],
            )
            self._ok(
                "TC-CHAT-02 reply avoids Conversation Summary heading",
                "conversation summary" not in lowered,
                text[:160],
            )
            self._ok(
                "TC-CHAT-03 reply avoids User request meta label",
                "user request" not in lowered,
                text[:160],
            )
        except RuntimeError as exc:
            self._ok("Live chat style scenario", False, str(exc)[:200])
    ...
    def run(self) -> bool:
        ...
        if self.custom.custom_only and has_custom:
            ...
            mode = "LIVE (OpenRouter API)"
        if has_custom and not self.custom.custom_only:
            mode += " + built-in suite"

        print("=" * 80)
        print("OpenRouter AI Connector — RPC / HTTP Test (Odoo 19)")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="RPC / HTTP automation test for connect_openrouter_ai (Odoo 19)",
    )
    ...
    parser.add_argument(
        "--live",
        action="store_true",
        help="Run live OpenRouter AI scenarios (context, RAG, HTTP). Requires API key.",
    )
    parser.add_argument(
        "--openrouter-key",
        default=None,
        help="OpenRouter API key to store in connect_openrouter_ai.api_key before live tests",
    )
    parser.add_argument(
        "--rag-timeout",
        type=int,
    ...
    custom.add_argument(
        ...
        help=f"OpenRouter model when creating a temporary agent (default: {DEFAULT_LIVE_LLM})",
    )
    custom.add_argument(
        "--custom-only",
        action="store_true",
...
def main() -> int:
    ...
    success = ConnectOpenRouterAIRPCTest(
        client,
        live=live,
