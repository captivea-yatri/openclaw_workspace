"""Shared helpers for sale-order scenarios."""
...
class SoScenarioBase:
    """Common partner / offer / SO helpers."""
    ...
    def _find_two_service_products(
        self, rpc: OdooRPCClient, offer_id: int, prefer_prepaid: bool = False
    ) -> list[dict]:
        fields = ["id", "name", "uom_id", "lst_price", "minimumSalePrice", "service_policy"]
        ...
        has_policy = "service_policy" in fg
        if not has_policy:
        ...
        products = rpc.search_read("product.product", base_domain, fields, limit=20)
        if prefer_prepaid and has_policy:
            prepaid = [p for p in products if p.get("service_policy") == "ordered_prepaid"]
            if len(prepaid) >= 2:
                return prepaid[:2]
        if len(products) < 2:
