...
# -*- coding: utf-8 -*-
"""
Merged test runner for:
- full_inter_company_transaction_flow.py
- test_product_account_restriction_rpc.py
- test_ksc_auto_invoice_rpc.py

Runs each script against the same Odoo instance, streams their full output,
extracts a "Result: X passed, Y failed" line (if present) and prints a
...
SCRIPTS = [
    {
        "path": Path(__file__).parent.parent / "full_inter_company_transaction_flow.py",
        "args": ["--url", ODOO_URL, "--db", DB, "--user", USER, "--password", PASSWORD],
        "label": "Inter‑Company Transaction Flow",
    },
    {
        "path": Path(__file__).parent / "test_product_account_restriction_rpc.py",
        "args": ["--url", ODOO_URL, "--db", DB, "--user", USER, "--password", PASSWORD, "--protocol", PROTOCOL],
        "label": "Product & Account Restriction",
    },
    {
        "path": Path(__file__).parent / "test_ksc_auto_invoice_rpc.py",
        "args": ["--url", ODOO_URL, "--db", DB, "--user", USER, "--password", PASSWORD, "--protocol", PROTOCOL],
...
def run_script(script_path: Path, args: list[str], label: str) -> dict:
    """Execute a test script, capture its output, and parse PASS/FAIL counts.
    ...
    try:
        ...
        return {"label": label, "passed": 0, "failed": 0, "total": 0, "output": "TIMEOUT after 600 s"}
...
def main() -> int:
    ...
    for script in SCRIPTS:
        ...
        results.append(res)
        print(res["output"])  # raw log for each script

    # Consolidated human‑readable summary
