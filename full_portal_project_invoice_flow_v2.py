...
1. Sets default_domain_ids on project creation (not after via write)
...
def main():
    ...
    task_id_from_wizard = None
    try:
        ctx = {"active_model": "project.project", "active_id": project_id}
        # Pass more fields to the wizard to help it create the task properly
        wizard_vals = {
            "project_id": project_id,
            "phase_id": phase_id,
            # Try adding domain and role if the wizard expects them
            "domain_id": domain_id if domain_id else False,
            "role_id": role_id if role_id else False,
        }
        ...
        try:
            ...
            # Even if it returns None, consider it successful if no fault
            wizard_success = True
    ...
    if wizard_success:
        ...
        if not task_id:
            ...
            wizard_success = False  # Trigger fallback
        else:
            task_id_from_wizard = task_id
    ...
    if not wizard_success:
        ...
        # Use the task ID we got from the wizard search
        task_id = task_id_from_wizard
