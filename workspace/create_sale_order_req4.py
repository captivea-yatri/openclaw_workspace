...
def main():
    ...
    if not uid:
        ...
        sys.exit(1)
    # user company
    user_rec = execute_kw(db, uid, pwd, 'res.users', 'read', [[uid], ['company_id']])
    company_id = user_rec[0]['company_id'][0]
    # product
    prod_name = 'Test Service'
    prod_ids = execute_kw(db, uid, pwd, 'product.product', 'search', [[('name', '=', prod_name)]], {'limit': 1})
    if prod_ids:
        prod_id = prod_ids[0]
    else:
    ...
    if partner_ids:
        ...
        partner_id = execute_kw(db, uid, pwd, 'res.partner', 'create', [{'name': 'Demo Customer', 'email': email}])
    ...
    vals['order_line'] = [(0, 0, {'product_id': prod_id, 'product_uom_qty': 1, 'price_unit': 100.0})]
    order_id = execute_kw(db, uid, pwd, 'sale.order', 'create', [vals])
    print(f'Sale order created {order_id}')
