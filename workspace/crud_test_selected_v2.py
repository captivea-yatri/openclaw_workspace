'''CRUD test for selected Odoo models (robust version).

- Loads credentials from `odoo_credentials.json`.
- Authenticates as the configured user.
- For each model in the provided list it attempts:
  1. READ (search limit 1)
  2. CREATE (populate first required field with a dummy value)
  3. WRITE (update that field on the created record)
  4. DELETE (remove the created record)
- If the model does not exist, the fault is captured and marked as `model_not_found`.
- Results are written to `crud_selected_report_v2.json`.
''' 
...
# password key may be masked; grab the first non‑standard key.
...
for model in model_list:
    ...
    if isinstance(read_ids, dict) and ("__fault__" in read_ids or "__error__" in read_ids):
        ...
        result = {"status": "model_not_found", "details": read_ids}
        report[model] = result
        continue
    else:
...
# Write report
out_path = os.path.join(os.path.dirname(__file__), "crud_selected_report_v2.json")
