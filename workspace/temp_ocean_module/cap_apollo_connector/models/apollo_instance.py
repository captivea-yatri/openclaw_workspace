...
class ApolloInstance(models.Model):
    ...
    @staticmethod
    def _encode_apollo_query_params(payload, array_keys=None):
        """Apollo endpoints expect filters as URL query parameters."""
        array_keys = set(array_keys or [])
        params = []
        for key, value in (payload or {}).items():
            if value is None or value == "":
                continue
            if isinstance(value, (list, tuple)):
                param_key = "%s[]" % key if key in array_keys or key.endswith("_ids") else key
                for item in value:
                    if item not in (None, ""):
    ...
    def search_people(self, payload=None):
        """Search Apollo database (POST /mixed_people/api_search). Does not consume credits."""
        self.ensure_one()
        params = self._encode_apollo_query_params(
            payload or {},
            array_keys=self._PEOPLE_SEARCH_ARRAY_KEYS,
        )
        _logger.info("Apollo people search params: %s", params)
        return self.apollo_api_request("POST", "/mixed_people/api_search", params=params)

    def bulk_enrich_people(self, person_ids, params=None):
        """Bulk enrich up to 10 people (POST /people/bulk_match). Consumes credits."""
        self.ensure_one()
        person_ids = [pid for pid in (person_ids or []) if pid]
        if not person_ids:
            raise UserError(_("Select at least one person to enrich."))
        if len(person_ids) > 10:
            raise UserError(_("Bulk enrichment supports up to 10 people per request."))

        options = dict(params or {})
        if options.get("reveal_phone_number"):
            webhook_url = (options.get("webhook_url") or self.enrichment_webhook_url or "").strip()
            if not webhook_url:
                raise UserError(
                    _(
                        "Phone reveal requires an HTTPS webhook URL on the Apollo instance."
                    )
                )
            options["webhook_url"] = webhook_url

        query_params = self._encode_apollo_query_params(options)
        json_body = {"details": [{"id": person_id} for person_id in person_ids]}
        _logger.info("Apollo bulk enrich for %s people", len(person_ids))
        return self.apollo_api_request(
            "POST",
            "/people/bulk_match",
            params=query_params,
            json_body=json_body,
    ...
    def action_open_lead_prospect(self):
        self.ensure_one()
        return {
            "name": _("Apollo Lead Prospecting"),
