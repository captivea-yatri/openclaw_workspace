...
def main():
    ...
    try:
        partner_ids = ek('res.partner', 'search', [[['name', '=', partner_name]]], {'limit': 1})
    except Exception as e:
    ...
    # Try to get the standard todo type via ir.model.data
    try:
        act_type_ids = ek('ir.model.data', 'search_read', 
                          [[['model', '=', 'mail.activity.type'], ['name', '=', 'data_todo']]], 
                          {'fields': ['res_id'], 'limit': 1})
    except Exception as e:
        print(f'Error searching for activity type via ir.model.data: {e}')
        act_type_ids = []

    if act_type_ids:
        activity_type_id = act_type_ids[0]['res_id']
        print(f'Found activity type id via ir.model.data: {activity_type_id}')
    else:
    ...
    # We don't have a flag, so we'll check if the partner we created is the only one with that name?
    # For simplicity, we'll skip cleanup for now.
    print('Done.')
